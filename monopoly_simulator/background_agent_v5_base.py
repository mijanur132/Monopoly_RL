...
"""Agent v5
Based on agent v4 who aggressively buy/bid/trade railroads, Boardwalk, and Park Place, this agent would like to
complete full color set of (St.James/Tennessee/New York) and/or (Oriental/Vermont/Connecticut) and build only 3
houses only in order to gain highest profits.
"""
...
def make_pre_roll_move(player, current_gameboard, allowable_moves, code):
    ...
    if player.agent.action is not None:
        #print("bav5,70.agent action:",player.agent.action)
        # player.agent.successful_actions += 1
        player.agent.next_state = rl_agent_helper.get_state_vector2(current_gameboard, player).to(player.agent.device)
        player.agent.reward = rl_agent_helper.get_reward_v5(current_gameboard, player).to(player.agent.device)
...
def make_buy_property_decision(player, current_gameboard, asset):
    ...
    decision = False
    if asset.name in properties_you_wanted and player.current_cash >= asset.price:
...
def make_bid(player, current_gameboard, asset, current_bid):
    ...
    if asset.name in properties_you_wanted and player.current_cash >= current_bid:
...
def _ask_for_improve_property(player, current_gameboard, code):
    ...
    """
    param = agent_helper_functions.identify_improvement_opportunity_limit(player, current_gameboard, num=3)
...
def _ask_for_make_trade_offer(player, current_gameboard):
    ...
    # 2. Aggressively to buy railroads
    elif player.num_railroads_possessed > 0 or 'SkyBlue' not in player.full_color_sets_possessed or \
            'Orange' not in player.full_color_sets_possessed:
...
properties_you_wanted = ('St. James Place', 'Tennessee Avenue', 'New York Avenue',
