...
from monopoly_simulator import \
    agent_helper_functions_v2  # helper functions are internal to the agent and will not be recorded in the function
# log.
import logging
import random
import torch
...
def get_output_action_v4(player, current_gameboard, allowable_moves, code, move):
    ...
    if len(action_idxes) > 1:
        ...
        action_to_take, params, action_failed = get_action_vector_mapping(action_idxes[0], player, allowable_moves,
                                                                              current_gameboard)
        # print("here")
        player.agent.successful_actions += 1
