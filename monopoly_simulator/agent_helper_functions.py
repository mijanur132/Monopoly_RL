...
# hyyyy
def identify_improvement_opportunity_limit(player, current_gameboard, num=3):
    potentials = list()
    for c in player.full_color_sets_possessed:
        c_assets = current_gameboard['color_assets'][c]
        for asset in c_assets:
            if can_asset_be_improved(asset, c_assets) and asset.price_per_house <= player.current_cash:
                if asset.num_houses < num:
                    logger.debug(f'{player.player_name}: {asset.name} currently have {asset.num_houses} < {num}, add to potential list.')
                    potentials.append((asset, asset_incremental_improvement_rent(asset) - asset.price_per_house))
    if potentials:
        sorted_potentials = sorted(potentials, key=lambda x: (x[1], x[0].name), reverse=True)
        param = dict()
        param['player'] = player
...
#hyyyy
def identify_property_trade_wanted_from_player_aggressively(player, current_gameboard, properties_agent_want):
    """
    A specific strategy func to identify properties for making monopoly, and also aggressively trade for
    Boardwalk, Park Place, all railroads, and etc.
