from monopoly_simulator import agent_helper_functions_old # helper functions are internal to the agent and will not be recorded in the function log.
...
def make_post_roll_move(player, current_gameboard, allowable_moves, code):
    ...
    if "buy_property" in allowable_moves:
        ...
        if make_buy_property_decision(player, current_gameboard, current_location):
            ...
            if agent_helper_functions_old.will_property_complete_set(player, current_location, current_gameboard):
                # if we can raise enough money, then the 'next' time around we'll succeed in buying
                to_mortgage = agent_helper_functions_old.identify_potential_mortgage(player, current_location.price, True)
                if to_mortgage:
                    ...
                    to_sell = agent_helper_functions_old.identify_potential_sale(player, current_gameboard, current_location.price, True)
                    if to_sell:
...
def make_buy_property_decision(player, current_gameboard, asset):
    ...
    if asset.name in properties_you_dont_wanted:
        ...
        decision = True
            ...
            agent_helper_functions_old.will_property_complete_set(player, asset, current_gameboard):
        logger.debug(player.player_name+ ': I will attempt to buy '+ asset.name+ ' from the bank.')
        decision = True
...
def make_bid(player, current_gameboard, asset, current_bid):
    ...
    elif current_bid < player.current_cash and agent_helper_functions_old.will_property_complete_set(player, asset, current_gameboard):
...
def _ask_for_accept_trade_offer(player, current_gameboard):
    ...
    if (player.outstanding_trade_offer['cash_offered'] <= 0 and len(player.outstanding_trade_offer['property_set_offered'])==0) \
        ...
        if reject_flag == 0:
            ...
            for prop in player.outstanding_trade_offer['property_set_offered']:
                if agent_helper_functions_old.will_property_complete_set(player, prop, current_gameboard):
...
def _ask_for_improve_property(player, current_gameboard, code):
    ...
    """
    param = agent_helper_functions_old.identify_improvement_opportunity(player, current_gameboard)
    if param:
...
def _make_trade_offer(player, current_gameboard, purpose_flag, want_properties_aggressively=False):
    ...
    if want_properties_aggressively:
        potential_request_list = agent_helper_functions_old.identify_property_trade_wanted_from_player_aggressively(player, current_gameboard, properties_you_wanted)
    else:
        potential_request_list = agent_helper_functions_old.identify_property_trade_wanted_from_player(player, current_gameboard)
    potential_offer_list = agent_helper_functions_old.identify_property_trade_offer_to_player(player, current_gameboard)
    param_list = agent_helper_functions_old.curate_trade_offer_multiple_players(player, potential_offer_list, potential_request_list, current_gameboard, purpose_flag=purpose_flag)
