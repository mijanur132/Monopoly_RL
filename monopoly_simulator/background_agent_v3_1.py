from monopoly_simulator import agent_helper_functions_v2 # helper functions are internal to the agent and will not be
...
def make_out_of_turn_move(player, current_gameboard, allowable_moves, code):
    ...
    if "accept_trade_offer" in allowable_moves:
        ...
        if (player.outstanding_trade_offer['cash_offered'] <= 0 and len(player.outstanding_trade_offer['property_set_offered'])==0) and \
            ...
            if reject_flag == 0:
                ...
                for prop in player.outstanding_trade_offer['property_set_offered']:
                    if agent_helper_functions_v2.will_property_complete_set(player,prop,current_gameboard):
    ...
    if "accept_sell_property_offer" in allowable_moves:
        ...
        elif agent_helper_functions_v2.will_property_complete_set(player, player.outstanding_property_offer['asset'],
                                                                current_gameboard):
    ...
    if player.status != 'current_move': # these actions are considered only if it's NOT our turn to roll the dice.
        if "improve_property" in allowable_moves: # beef up full color sets to maximize rent potential.
            param = agent_helper_functions_v2.identify_improvement_opportunity(player, current_gameboard)
            if param:
        ...
        if player.current_cash < current_gameboard['go_increment'] and "make_trade_offer" in allowable_moves:
            ...
            potential_offer_list = agent_helper_functions_v2.identify_property_trade_offer_to_player(player,
                                                                                                   current_gameboard)
            potential_request_list = agent_helper_functions_v2.identify_property_trade_wanted_from_player(player,
                                                                                                        current_gameboard)
            param_list = agent_helper_functions_v2.curate_trade_offer_multiple_players(player, potential_offer_list,
                                                                                     potential_request_list, current_gameboard, purpose_flag=1)
            #logger.debug(param)
            return_action_list = []
            return_param_list = []
            ...
            potential_request_list = agent_helper_functions_v2.identify_property_trade_wanted_from_player(player,
                                                                                                        current_gameboard)
            param_list = agent_helper_functions_v2.curate_trade_offer_multiple_players(player, potential_offer_list,
                                                                                     potential_request_list, current_gameboard, purpose_flag=2)
            #logger.debug(param)
            return_action_list = []
            return_param_list = []
...
def make_post_roll_move(player, current_gameboard, allowable_moves, code):
    ...
    if "buy_property" in allowable_moves:
        ...
        if make_buy_property_decision(player, current_gameboard, current_location):
            ...
            if agent_helper_functions_v2.will_property_complete_set(player,current_location,current_gameboard):
                # if we can raise enough money, then the 'next' time around we'll succeed in buying
                to_mortgage = agent_helper_functions_v2.identify_potential_mortgage(player,current_location.price,True)
                if to_mortgage:
                    ...
                    to_sell = agent_helper_functions_v2.identify_potential_sale(player, current_gameboard,
