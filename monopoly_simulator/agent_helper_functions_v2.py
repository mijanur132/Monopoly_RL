...
def identify_improvement_opportunity(player, current_gameboard):
    ...
    for c in player.full_color_sets_possessed:
        ...
        for asset in c_assets:
            if can_asset_be_improved(asset, c_assets, current_gameboard) and asset.price_per_house <= player.current_cash: # player must be able to afford the improvement
                potentials.append((asset, asset_incremental_improvement_rent(asset, current_gameboard) - asset.price_per_house))
    if potentials:
        ...
        if (param['asset'].num_houses == current_gameboard['bank'].house_limit_before_hotel) or (0 < param['asset'].num_hotels < current_gameboard['bank'].hotel_limit):
            param['add_hotel'] = True
            param['add_house'] = False
        if (param['add_house'] and current_gameboard['bank'].total_houses > 0) or (param['add_hotel'] and current_gameboard['bank'].total_hotels > 0):
            return param
        else:
            logger.debug("Wanted to improve " + param['asset'].name + ". But Bank does not have sufficient # of houses/hotels needed for improvement, cannot improve!")
            return None
    else:
        return None
...
def can_asset_be_improved(asset, same_color_assets, current_gameboard):
    """
    ...
    if asset.loc_class != 'real_estate' or asset.is_mortgaged:
        return False
    if asset.num_hotels > current_gameboard['bank'].hotel_limit - 1:
        return False # we can't improve any further
    if asset.num_houses == 0 and asset.num_hotels == 0:
        for c_asset in same_color_assets:   # if a hotel was sold, num_houses will be zero, hence cannot improve if there are hotels on other properties of same color
            if c_asset != asset and c_asset.num_hotels != 0:
                return False
        return True
    count = 0
    count_hotel = 0
    for c_asset in same_color_assets:
        ...
        if c_asset == asset:
            continue
        if c_asset.num_hotels == 1 and asset.num_houses == current_gameboard['bank'].house_limit_before_hotel:
            return True # we can build a hotel on asset
        if asset.num_hotels > 0 and c_asset.num_hotels > asset.num_hotels:    # in case the bank.hotel_limit is > 1, the players can set up > 1 hotel per property
            return True
        if c_asset.num_houses > asset.num_houses:
            return True
        if asset.num_houses > 0 and c_asset.num_houses == asset.num_houses:
            count += 1
        if asset.num_hotels > 0 and c_asset.num_hotels == asset.num_hotels:
            count_hotel += 1
    ...
    if (count_hotel == len(same_color_assets) - 1) and (asset.num_hotels < current_gameboard['bank'].hotel_limit): # every asset in same_color has the same
        # no. of hotels as the current asset and this number is < hotel_limit, hence it can be improved (by building another hotel).
        return True

    return False


def asset_incremental_improvement_rent(asset, current_gameboard):
    """
    ...
    if 0 < asset.num_hotels < current_gameboard['bank'].hotel_limit:
        return 0
    elif asset.num_houses == current_gameboard['bank'].house_limit_before_hotel:     # currently if num houses on a prop > 4, rent remains same
        return asset.rent_hotel-asset.rent_4_houses
    elif 4 <= asset.num_houses < current_gameboard['bank'].house_limit_before_hotel:
        return 0        # no incremental adv for additional houses
        ...
        return asset.rent_1_house - (asset.rent*current_gameboard['bank'].monopolized_property_rent_factor)
                                    # remember, if the house can be improved, then it is monopolized, so twice the rent is being charged even without houses.
